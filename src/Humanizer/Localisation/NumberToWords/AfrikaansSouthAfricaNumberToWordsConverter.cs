...
{
    internal class AfrikaansSouthAfricaNumberToWordsConverter : GenderlessNumberToWordsConverter
    {
        ...
        private string Convert(int number, bool isOrdinal)
        {
            ...
            if (number > 0)
            {
                ...
                else
                {
                    ...
                    {
                        lastPart =
                            $"{GetUnitValue(number % 10, false)} en {(isOrdinal ? GetUnitValue(lastPartValue, isOrdinal) : lastPart)}";
                    }
                    else if ((number % 10) == 0)
                    {
